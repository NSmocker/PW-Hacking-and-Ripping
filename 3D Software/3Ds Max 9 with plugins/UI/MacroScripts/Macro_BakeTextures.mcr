...
local	mapPresets = #(128,256,512,768,1024,2048) -- NxN map size presets - array size must be 6

local 	skip_DX9_materials = productAppID != #max  -- true -- set to true to skip DX9 materials as new baked output materials
...
local 	InitListView, GetLvSelection, SelectLvItem, GetLvItemName, SetLvItemName, GetLvItemCheck, SetLvItemCheck, SetLvItemRowColor, AddLvItem,
		DeleteLvItem 
...
(
	...
	return (RTT_data.emptyTargetsOk != 0)
...
----------------------------------------------------------------------------
--
--	Routines to handle checking of exposure control
--
-- message box to confirm render with Auto or Linear exp control
rollout ExposureControlOKBox "ExposureControl Warning" width:370 height:120
(
	label errTxt1 "" align:#left
	label errTxt2 "This exposure control calculates the exposure of each object separately, " align:#left offset:[0,-4]
	label errTxt3 "which is not desirable because each object will have a different " align:#left offset:[0,-4]
	label errTxt4 "exposure and things will never render correctly when put together." align:#left offset:[0,-4]
	button bCancel "Cancel Render" width:92 height:24 across:2 align:#right offset:[-10,2]
	button bContinue "Continue" width:87 height:24 align:#left offset:[10,2]
	checkbox cNotAgain "Don't show again" width:120 height:18 checked:false 
	
	on ExposureControlOKBox close do
	(	
		pExposureControlOKBoxPos = GetDialogPos ExposureControlOKBox
	)
	on ExposureControlOKBox open do
	(	
		local expc_class = classof SceneExposureControl.exposureControl
		errTxt1.text = "Warning: You are using " +  expc_class.localizedname + "."
	)
	on bCancel pressed do 
	(
		RTT_data.exposureControlOK = 0
		destroydialog ExposureControlOKBox
	)
	on bContinue pressed do
	(
		RTT_data.exposureControlOK = if cNotAgain.checked then 2 else 1
		destroydialog ExposureControlOKBox
	)
) -- end, exposure control dialog
...
(
	...
	(
		if RTT_data.exposureControlOK < 2 then -- if 0 or 1 ...
			createDialog ExposureControlOKBox modal:true pos:pExposureControlOKBoxPos
...
(
	...
	-- the VFB is not needed when using mental ray
	...
	(
		...
		(	
			...
			if _debug do format "conditional for CollectUpdateFiles - obj.material:%; allowCreateMtl:%; useObjBakeForMtl:%; useSubObjBakeForMtl:%; subObjBake:%\n" obj.material allowCreateMtl useObjBakeForMtl useSubObjBakeForMtl subObjBake
...
(
	...
	(
		...
		InitListView lvObjects 5 #("","Name","Object Channel","Sub-Object Channel","Edge Padding") #(0,161,50,56,44) checkBoxes:false -- init the active x list view
		lvObjects.sortKey = 1
	...
	(	
		...
		(	
			...
			(
				...
				li.listSubItems[1].text = theName 
				li.listSubItems[2].text = channel_Obj
				li.listSubItems[3].text = channel_SubObj
				li.listSubItems[4].text = padding
			)
		)
...
(
	...
	--	GroupBox gElementSettings "Selected Element Common Settings" width:318 height:131 enabled:false
		...
		spinner sWidth "Size: " width:86 height:16 enabled:false range:[0,8192,256] type:#integer fieldwidth:40 align:#right across:4 offset:[10,0]
...
--		spinner sHeight "Height: " width:89 height:16 enabled:false range:[0,8192,256] type:#integer fieldwidth:40 align:#right across:4 offset:[10,0]
		label lHeight "" across:4 offset:[0,0]
	...
	(
		...
		if workingObjects.count == 1 then
			UpdateElementSz sWidth.value
		else
			UpdateElementSz undefined
	)
	...
	(
		sWidth.value = mapPresets[presetIndex]
		cAutoSz.checked = false
		UpdateElementSz sWidth.value
	)
	...
	function AddElementToList _on _fileName _eleName _sz _target _tag=
	(
		local size
			
		if _sz == undefined then size = "Varies" 
		else size = _sz as string
	
		AddLvItem lvElements #(_fileName,_eleName,size,_target) checked:_on tag:_tag -- returns listItem
	)
	
	function UpdateElementSz _sz  =
	(
		local size
			...
			(
				...
				(
					...
					(
						...
						addElementToList isOn.value "" eleNames[i] szX.value target index
	...
	--->>>>>>	-- single object
				...
				(
					...
					(
						...
						addElementToList isOn element.fileName element.elementName element.outputSzX element.targetMapSlotName i
	...
	(
		if _debug2 do format "Close Element\n"  
		if _debug2 do format "close selected elements - selectedElement: %; selectedElementIndex: %\n" selectedElement selectedElementIndex
		...
		(
			...
			(
				local ele = e.element
				if _debug2 do format "Close Element: % \n" e -- eName.text 
				if _debug2 do format "ele.elementName: %; eName.text: %\n" ele.elementName eName.text 
				if( ele.elementName != eName.text ) then
				(
					if _debug2 do format "workingObjects.count: %\n" workingObjects.count
					ele.elementName = eName.text
	...
	(
		...
		(
			...
			sWidth.indeterminate = (workingObjects.count > 1)
	...
	(
		...
		local name, fName, isAutoSz, szX, target, allTargetsForSelection
		local isOn = triStateValue()
		...
		(
			...
			(
				...
				selectedElement.outputSzY = selectedElement.outputSzX
...
(
	...
	(
		...
		checkbutton bHeightMinPick "" width:18 height:18 offset:[-16,-1] \
			images:#("$ui\Icons\MeditTools_i.bmp", "$ui\Icons\MeditTools_a.bmp", 43,41,41,41,41, true)
